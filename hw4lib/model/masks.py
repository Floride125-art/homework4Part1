...
def PadMask(padded_input, input_lengths):
    ...
    positions = torch.arange(seq_length, device=padded_input.device).expand(batch_size, seq_length)
...
def CausalMask(padded_input):
    ...
    seq_length = padded_input.shape[1]
    mask = torch.triu(
        torch.ones(seq_length, seq_length, device=padded_input.device, dtype=torch.bool), 
        diagonal=1
    )
    return mask
